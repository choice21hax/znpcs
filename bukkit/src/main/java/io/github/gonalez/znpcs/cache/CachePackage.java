...
public enum CachePackage {
  DEFAULT,
  CRAFT_BUKKIT("org.bukkit.craftbukkit" + (Utils.getBukkitPackage().isEmpty() ? "" : "." + Utils.getBukkitPackage())),
  ...
  CachePackage(String packageName) {
    this.fixedPackageName = Utils.versionNewer(17)
        ? packageName
        : packageName + (packageName.contains("minecraft") ? ".server." + Utils.getBukkitPackage() : "");
  }
